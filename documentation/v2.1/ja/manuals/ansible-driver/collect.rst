...
`はじめに 3 <#_Toc101979278>`__

`1 収集機能概要 4 <#収集機能概要>`__

`1.1 収集機能について 4 <#収集機能について>`__

`1.1.1 収集機能概要図 4 <#収集機能概要図>`__

`1.1.2 収集機能データ登録処理概要図 5 <#収集機能データ登録処理概要図>`__

`1.2 パラメータシートへの登録方法について
6 <#パラメータシートへの登録方法について>`__

`1.2.3 収集機能の動作要件 6 <#収集機能の動作要件>`__

`2 収集機能でのディレクトリ、ファイル構造、変数取り扱い
7 <#収集機能でのディレクトリファイル構造変数取り扱い>`__

`2.1 収集対象ディレクトリ、ファイル構造
7 <#収集対象ディレクトリファイル構造>`__

`2.1.1 収集対象のファイルフォーマット
7 <#収集対象のファイルフォーマット>`__

`2.1.2 収集対象ディレクトリ構造 7 <#収集対象ディレクトリ構造>`__

`2.2 取り扱う変数と種類 10 <#取り扱う変数と種類>`__

`3 収集機能 コンソールメニュー構成 11 <#_Toc101979290>`__

`3.1 メニュー/画面一覧 11 <#メニュー画面一覧>`__

`4 収集機能の利用手順 13 <#収集機能の利用手順>`__

`4.1 作業フロー 13 <#_Toc101979293>`__

`4.1.1 収集機能作業フロー 13 <#収集機能作業フロー>`__

`5 収集機能・操作方法説明 15 <#収集機能操作方法説明>`__

`5.1 Ansible 共通コンソール 15 <#ansible-共通コンソール>`__

`5.1.1 収集インターフェース情報 15 <#収集インターフェース情報>`__

`5.1.2 収集項目値管理 17 <#収集項目値管理>`__

`5.2 Ansible-Legacy、Ansible-Pioneer、Ansible-LegacyRoleコンソール
20 <#ansible-legacyansible-pioneeransible-legacyroleコンソール>`__

`5.2.1 収集状況の確認 20 <#収集状況の確認>`__

`5.3 BackYardコンテンツ 22 <#backyardコンテンツ>`__

`5.3.1 パラメータシートへの登録処理の概要
22 <#パラメータシートへの登録処理の概要>`__

`6 運用操作 23 <#運用操作>`__

`6.1 メンテナンス 23 <#メンテナンス>`__

`6.2 メンテナンス方法について 23 <#メンテナンス方法について>`__

`7 付録 25 <#_Toc101979306>`__

`7.1 参考資材 25 <#参考資材>`__

`7.2 収集実行例 27 <#収集実行例>`__

`7.2.1 複数ファイルの同一メニューを対象とした場合
27 <#複数ファイルの同一メニューを対象とした場合>`__

`7.2.2 収集対象ファイルの値の取り扱い
28 <#収集対象ファイルの値の取り扱い>`__

`7.2.3 複数の同一ファイル名がアップロード対象ファイルの指定例
32 <#複数の同一ファイル名がアップロード対象ファイルの指定例>`__

`7.2.4 ファイル削除時の収集対象ファイル内の記載例
...
| **ITA独自変数**            | **変数指定内容**           | **備考** |
+============================+============================+==========+
| \__parameter_dir_\_        | 作業結果ディレクトリ配     |          |
|                            | 下の「_parameters」のパス  |          |
+----------------------------+----------------------------+----------+
| \__parameters_file_dir_\_  | 作業結果ディレクトリ配下の |          |
|                            | 「_parameters_file」のパス |          |
+----------------------------+----------------------------+----------+
| \_                         | 作業ディレクトリ配         |          |
| _parameters_dir_for_epc_\_ | 下の「_parameters」のパス  |          |
+----------------------------+----------------------------+----------+
| \__para                    | 作業ディレクトリ配下の     |          |
| meters_file_dir_for_epc_\_ | 「_parameters_file」のパス |          |
+----------------------------+----------------------------+----------+
...
|                    | **識別子**   |                    |          |
+====================+==============+====================+==========+
| Ansible-Legacy     | legacy/ns/   | /データリレイ      |          |
|                    |              | ストレージパス(An  |          |
|                    |              | sible）/legacy/ns/ |          |
+--------------------+--------------+--------------------+----------+
| Ansible-Pioneer    | pioneer/ns/  | /データリ          |          |
|                    |              | レイストレージパス |          |
|                    |              | (Ansible）/pioneer |          |
|                    |              | /ns/               |          |
+--------------------+--------------+--------------------+----------+
| Ansible-LegacyRole | legacy/rl/   | /データリレイ      |          |
|                    |              | ストレージパス(An  |          |
|                    |              | sible）/legacy/rl/ |          |
+--------------------+--------------+--------------------+----------+
...
+------------------+--------------------------------------+----------+
| **指定方式**     | **YAMLファイルへの記載方法**         | **備考** |
+==================+======================================+==========+
| ファイル名指定   | VAR_FILE_NAME : <‘ファイル名>’       |          |
+------------------+--------------------------------------+----------+
| ファイルパス指定 | VAR_FILE_NAME :                      |          |
|                  | ‘/                                   |          |
| (完全一致)       | <上位ディレクトリ>/_parameters_file/ |          |
|                  | localhost/<階層X>/<ファイル名>’      |          |
+------------------+--------------------------------------+----------+
| ファイルパス指定 | VAR_FILE_NAME :                      |          |
|                  | ‘/<階層X>/<ファイル名>’              |          |
...
+==================================+==================================+
| パラメータ自動登録               | ky_s                             |
|                                  | td_synchronize-Collector.service |
| 作業実                           |                                  |
| 行が完了した、作業結果から、設定 |                                  |
| 項目値管理に登録した情報を元に、 |                                  |
| パラメータシートへの登録を行う。 |                                  |
...
8.     VAR_true       true    
9.     VAR_false      false   
10.    VAR_STR_true   "true"  
11.    VAR_STR_false  "false" 
12.    VAR_YES        YES     
13.    VAR_NO         NO      
14.    VAR_STR_YES    "YES"   
15.    VAR_STR_NO     "NO"    
...
+--------+--------------+--------------+-------------+-------------+
| **No** | **収集対象** | **           | **RESTAPI** | **WEB画面** |
|        |              | パラメータシ |             |             |
|        | **（         | ート項目名** |             |             |
|        | キー:値）**  |              |             |             |
+========+==============+==============+=============+=============+
| 1.     | VAR_TEST:    | パラメー     | "TEST"      | TEST        |
|        | TEST         | タ/VAR_TEST  |             |             |
+--------+--------------+--------------+-------------+-------------+
| 2.     | VA           | パラメータ/V | "TEST1"     | TEST1       |
|        | R_STR_TEST1: | AR_STR_TEST1 |             |             |
|        | 'TEST1'      |              |             |             |
+--------+--------------+--------------+-------------+-------------+
| 3.     | VA           | パラメータ/V | "TEST2"     | TEST2       |
|        | R_STR_TEST2: | AR_STR_TEST2 |             |             |
|        | "TEST2"      |              |             |             |
+--------+--------------+--------------+-------------+-------------+
| 4.     | VAR_null:    | パラメー     | null        |             |
|        | null         | タ/VAR_null  |             |             |
+--------+--------------+--------------+-------------+-------------+
| 5.     | VAR_NULL:    | パラメー     | null        |             |
|        | NULL         | タ/VAR_NULL  |             |             |
+--------+--------------+--------------+-------------+-------------+
| 6.     | V            | パラメータ/  | "null"      | null        |
|        | AR_STR_null: | VAR_STR_null |             |             |
|        | "null"       |              |             |             |
+--------+--------------+--------------+-------------+-------------+
| 7.     | V            | パラメータ/  | "NULL"      | NULL        |
|        | AR_STR_NULL: | VAR_STR_NULL |             |             |
...
|        | false        | タ/VAR_false |             |             |
+--------+--------------+--------------+-------------+-------------+
| 10.    | V            | パラメータ/  | "true"      | true        |
|        | AR_STR_true: | VAR_STR_true |             |             |
|        | "true"       |              |             |             |
+--------+--------------+--------------+-------------+-------------+
| 11.    | VA           | パラメータ/V | "false"     | false       |
|        | R_STR_false: | AR_STR_false |             |             |
|        | "false"      |              |             |             |
+--------+--------------+--------------+-------------+-------------+
| 12.    | VAR_YES: YES | パラメ       | "1"         | 1           |
|        |              | ータ/VAR_YES |             |             |
+--------+--------------+--------------+-------------+-------------+
| 13.    | VAR_NO: NO   | パラメ       | ""          |             |
|        |              | ータ/VAR_NO  |             |             |
+--------+--------------+--------------+-------------+-------------+
| 14.    | VAR_STR_YES: | パラメータ   | "YES"       | YES         |
|        | "YES"        | /VAR_STR_YES |             |             |
+--------+--------------+--------------+-------------+-------------+
| 15.    | VAR_STR_NO:  | パラメータ   | "NO"        | NO          |
|        | "NO"         | /VAR_STR_NO  |             |             |
+--------+--------------+--------------+-------------+-------------+
| 16.    | VAR_NON:     | パラメ       | null        |             |
|        |              | ータ/VAR_NON |             |             |
...
========================= ================================== ========
**収集項目(FROM)/変数名** **対象ファイル**                   **備考**
========================= ================================== ========
VAR_upload_file_1         、①、②、③、④のファイルからランダム 
VAR_upload_file_2         、②のファイルが対象                
VAR_upload_file_3         、②、④のファイルからランダム       
VAR_upload_file_4         、①のファイルが対象                
VAR_upload_file_5         、④のファイルが対象                
========================= ================================== ========
...
.. |image1| image:: collect/image1.png
   :width: 3.35069in
   :height: 0.78542in
.. |image2| image:: collect/image4.emf
   :width: 6.23056in
   :height: 5in
.. |image3| image:: collect/image7.png
   :width: 6.00372in
   :height: 2.87662in
.. |image4| image:: collect/image10.png
   :width: 5.65382in
   :height: 2.70023in
.. |image5| image:: collect/image11.png
   :width: 5.475in
   :height: 0.70833in
.. |image6| image:: collect/image12.png
   :width: 5.74716in
   :height: 2.75357in
.. |image7| image:: collect/image13.png
   :width: 5.47222in
   :height: 0.85139in
.. |image8| image:: collect/image7.png
   :width: 5.78688in
   :height: 2.77273in
...
.. |image13| image:: collect/image15.png
   :width: 6.06086in
   :height: 0.52834in
.. |image14| image:: collect/image14.png
   :width: 6.11686in
   :height: 2.26773in
.. |image15| image:: collect/image15.png
   :width: 6.09421in
   :height: 0.32738in
.. |image16| image:: collect/image16.png
   :width: 6.00785in
   :height: 0.51786in
.. |image17| image:: collect/image17.png
   :width: 6in
   :height: 0.52304in
.. |image18| image:: collect/image15.png
   :width: 6.06086in
   :height: 0.52834in
.. |image19| image:: collect/image18.png
   :width: 6.96252in
   :height: 1.98221in
